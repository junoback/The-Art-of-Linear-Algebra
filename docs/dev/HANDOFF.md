...
- [x] **S18 ch04 V-02 圖像模式 + Mona Lisa SVD 首輪交付** — Back 選 HANDOFF S17 推薦路線 A 開始圖像模式；S18 跳過外部 npy / base64 / fetch 不確定性，改「程式內 procedural 生成 4 張 64×64 灰階圖像」（gen_portrait 平滑背景 + 5 個 Gaussian 特徵 / gen_stripes 水平正弦 / gen_gradient 雙線性 / gen_random raw `standard_normal` mean 0）+ SVD 在 cell 內即時計算（~5 ms × 4 = ~20 ms）+ SVD_CACHE 字典；產出 [viz/ch04_matrix_matrix.py](../../viz/ch04_matrix_matrix.py) 從 440 行擴至 ~570 行 / 9 cell
- [x] **S18 mode radio + r_image slider + 5 cell branch on is_small** — mode = mo.ui.radio 5 選項（小矩陣 demo / Mona Lisa / 條紋 / 漸層 / 隨機）；r_image slider 0..64 與 r_small 0..2 分開（避免狀態混淆，切 mode 時不丟失對應 mode 的 r 值）；5 個輸出 cell（控制面板 / 即時計算 markdown / 主舞台 / strip / footer）全部 branch on `is_small = (mode.value == "小矩陣 demo")`
- [x] **S18 主舞台 + strip 圖像模式** — 主舞台圖像模式 1×3 灰階「原圖 M / 重建 M_r (r/k=64) / 誤差 |M-M_r|」（CMAP_GRAY = "Greys_r" 標準影像慣例 0=黑 1=白；dynamic zmin/zmax 適應每張圖值域），小矩陣模式維持 S17 的 2×3 共 6 heatmap；strip 圖像模式換 64 條 σ 譜柱狀圖（紫 #9467bd = 前 r 個已累加 / 灰 #bbbbbb = 尚未；vline 標 r 位置），小矩陣模式維持 k=2 layer 縮圖灰/亮兩態
- [x] **S18 本機 SVD 對拍 4 圖預期性質** — portrait σ_1=42.28 σ_2=2.54 σ_64≈0 eff_rank ~5（5 個 Gaussian 特徵 + 平滑背景）/ stripes σ_1=36.70 σ_2~1e-15 rank 1 精確（separable f(y)·1(x)）/ gradient σ_1=36.76 σ_2=1.51 σ_3~1e-15 rank 2 精確（bilinear ax+by+c）/ random σ_1=15.45 σ_2=14.65 σ_64=0.09 滿秩 Marchenko-Pastur（r=28 才達 85% 能量、r=49 才達 99%）
- [x] **S18 random 設計失誤自我修正** — 第一版 gen_random 用 min-max 標準化到 [0, 1]（為了與其他圖像顯示一致）→ 注入強 DC 分量讓 σ_1 ~ 34 主導（r=1 即 85% 能量）→ 完全破壞「random = 不可壓縮」教學意圖；發現後立即改為 raw `standard_normal` mean 0、σ 譜呈 Marchenko-Pastur 緩降，符合教學意圖；顯示用 dynamic zmin/zmax 對稱範圍視覺化（自動適應 [-3, 3] 範圍）
- [x] **S18 footer 表用實測數字校正** — 第一版 footer 觀察重點表用估計值「portrait ~10-20 effective rank」「random ~55」等 → 跑完本機 SVD 對拍發現 portrait 實為 5（不是 10-20），random r85=28（不是 55） → 校正為實測；加 DC 注意段（[0,1] 值域 σ_1 由 DC 主導，「累積能量 99% at r=1」是 Frobenius 觀點但視覺仍需 r≥2 看結構，這是 Strang LAFE §6.5/§7「Frobenius norm vs perceptual quality」議題）
- [x] **S18 WASM export + 部署驗證** — `.venv/bin/marimo export html-wasm ch04_matrix_matrix.py -o dist/ch04_matrix_matrix` → 27 MB static dir；marimo check 只噴 2 個 markdown-indentation cosmetic warning 不影響運行；HTTP 200 + http.server :8765 啟動給 Back 5 項驗證清單（1 小矩陣模式預設正常 / 2 切 Mona Lisa 三圖並列 / 3 拉 r slider σ 譜紫灰分界線移動 / 4 切條紋/漸層/隨機 σ 譜 shape 不同 / 5 console 無 STDERR）
- [x] **S18 SOP §2.15 補入 3 條新陷阱（合計 9 大）** — (7) **小資產 ≤ 100 KB 在 cell 內 procedural 生成避開 WASM file-loading 不確定性**（HANDOFF S17 預估的 base64/fetch 難題完全可繞過，notebook 自包含最穩） / (8) **[0, 1] 值域圖像 σ_1 由 DC 主導，累積能量百分比與視覺品質不對齊**（σ_1 ~ 42 為 mean 0.6 × 64 = 38.4 主導；σ_2 ~ 2.5 才捕捉特徵；「能量 99% at r=1」是 Frobenius 觀點但視覺仍需 r ≥ 2 看結構） / (9) **random / noise 不可 min-max 標準化到 [0, 1]**（會注入強 DC 分量讓 σ_1 主導；需保留 raw centered `standard_normal` 才有 Marchenko-Pastur σ 譜）
- [x] **S19 啟動「逆向設計第二骨架」 — Back 提出全書第二骨架** — 與 Appendix D「正向背後觀念 3-layer」對偶：D 橫切剖析「為什麼這條規則長這樣」，E 縱向走一遍「**怎麼從零反推出這條規則**」。核心信念：**矩陣運算（點積 / 外積 / 矩陣乘法 / 列空間 / 4 子空間 / 五大分解 / 偽反）沒有一個是憑空發明的，全部是「實際問題 + 還原需求」反推出來的唯一可能規則**
- [x] **S19 確立 5 步反推骨架** — (1) 實際問題（結果需求）/ (2) 物件化（抽離係數與變數，含「對 A 的操作必須對應方程組層的合法代數變形」可逆閉合條件）/ (3) 尋找未知運算 ◯（明確「◯ 是未知的，我們正在反推它」）/ (4) 閉合需求反推 ◯ 的規則（具體 → 一般 → boxed 公式）/ (5) 副產物自動冒出（規則確立後相關性質、概念、定理全部自動冒出）；5 步擴展為 **12 Step 標準段落**（Step 0 / 1 / 2 / 2 加強 / 3 / 4 / 5-7 副產物 / 8 雙路閉合 / 9 邊界條件 / 10 昇華 / 11 連結 / 12 Strang 鎖核）
- [x] **S19 R01 完整鏈條（食譜 2×3 主例題貫穿）** — Step 0 正向 vs 逆向視角對比 / Step 1 食譜問題（3 食材 × 2 營養指標 → 2×3 矩陣 A = [[2,3,1],[1,-1,4]]，$\mathbf{x}^* = (1,1,2)^{\top}$ → $\mathbf{b} = (7, 8)^{\top}$）/ Step 2 物件化 + 完整解集（1 維直線 $(1,1,2) + t(-13,7,5)$）/ Step 2 加強對 A 合法操作（3 ✓ ① 整條方程乘非零常數 / ② 兩條方程互換 / ③ 方程加倍 + 3 ❌ row 乘 0 / 只動 A 不動 b / entry 跨 row 互換）全公式可視化每步含「方程組層變形」+「A 與 b entry 級同步」+「驗證 $\mathbf{x}^*$」+「為何合法/破壞」+ **暗線埋伏指 R03 兌現** / Step 3 尋找 ◯ / Step 4 反推點積規則 boxed `(Ax)_i = (A row i) · x` / Step 5 維度規則自動冒出 / Step 6 (Mv2) 線組合自動冒出（同一規則另一展開方式）/ Step 7 列空間 + rank=2 + $\mathbf{c}_3 = \tfrac{13}{5}\mathbf{c}_1 - \tfrac{7}{5}\mathbf{c}_2$ + Rank-Nullity 2+1=3 數值驗證 / Step 8 雙路閉合 / Step 9 線性 vs 非線性邊界（$\mathbf{x}=(2,1,1)$ 反例 8 vs 12 差異 4 / Taylor + kernel + 神經網路混合） / Step 10 boxed 流程圖 / Step 11 R02-R07 鏈條樹 / Step 12 Strang LAFE §1.4 名言鎖核
- [x] **S19 R02 完整鏈條（紅蘿蔔 × 盤子 → 3×2 矩陣 M）** — 從 R01 (Mv2) 副產物追問「最小、最簡單的矩陣資訊單位是什麼」；主例題 u = (20, 40, 60)（單顆熱量 = 2 × mass）+ v = (4, 7)（盤子顆數），構造 M_ij = u_i × v_j；Step 4 反推外積 uv⊤ = 直行 × 橫行的結構（u 形狀 m×1 / v⊤ 形狀 1×n / M 形狀 m×n），逐 6 個 entry 驗算（80/140/160/280/240/420）；Step 5 形狀規則 / Step 6 rank 1 自動冒出（所有 column 都是 u 的倍數 / 所有 row 都是 v⊤ 的倍數）/ Step 7 矩陣秩 1 拆解（任何 m×n 矩陣 = 最多 n 個秩 1 矩陣之和 — MM4 列乘行讀法 + SVD 秩 1 累加之源）對 R01 主例題 A 驗算 3 個秩 1 之和 = A / Step 8 雙路閉合 / Step 9 為何 rank 1 是最小非零原子 / Step 10 boxed 流程圖 / Step 11 R03 兌現預告 / Step 12 Strang「秩 1 矩陣是所有矩陣的積木」鎖核
- [x] **S19 痛苦教訓：「Step 2 加強鐵律」明文化** — 第一稿犯邏輯循環錯誤：用「左乘可逆 M / elementary 矩陣 / LU 分解」描述合法操作 → Back 立即指出「跳太遠 邏輯循環」（要先懂矩陣乘法 = 整條鏈反推目標）→ 修訂改用「**方程組層代數語言**」（中學代數變形：方程乘 c / 互換 / 加倍）+ entry 級同步動作 + 暗線埋伏指 R03 兌現 → Back 確認「符合精神」；**鐵律規範：寫 R-chain Step 2 加強描述「對物件的合法操作」絕對不可使用尚未在 Step 4 反推出來的運算工具來解釋**；S19 三處同步明文化（SOP §2.16 + SCHEMA §3.7 + memory feedback_reverse_design.md）
- [x] **S19 主例題不切換規則明文化** — 第二稿時為簡化把主例題 2×3 切換到 2×2 副例題寫 Step 2a/2b → Back 制止「請想辦法延續用 Step 1 的例子不要換到 2×2」（認知斷裂、心智 context 重置成本高、破壞鏈條閱讀體驗）；**規範：R-chain 必須有一個主例題（含具體數字）貫穿 Step 1-9，不可中途切換**；S19 修訂後全程用食譜 2×3 主例題（Step 2 加強 6 個合法/非法操作 + Step 4-8 副產物驗算全部）
- [x] **S19 公式可視化粒度明文化** — Back 原話「R01 鏈需要把每一步你提到的操作都用公式推導可視化」；**規範：** 每個操作步驟必須含 boxed 公式 + 具體 entry 級數字 + 驗算對拍（不能只是文字描述）；R01 整體 ~35 個 `$$\boxed{...}$$` 公式塊 / ~20 個具體驗算
- [x] **S19 7 個檔案 ~1800 行新內容** — 新檔 [appendix-E-reverse.md](../book/appendix-E-reverse.md) 760 行（R01 ~530 行 + R02 ~230 行 + R03-R07 預留段）+ [BOOK.md](../book/BOOK.md) 12305 → **13068 行（+763 / 17 → 18 個 md 合一）**（fence-aware awk 降一級 append）+ [SCHEMA §3.7「逆向設計鏈規範」](../book/SCHEMA.md) +130 行（版本 0.4） + [VIZ-CATALOG.md Appendix E 索引段](../book/VIZ-CATALOG.md) +35 行 + [SOP §2.16「R-chain 寫作鐵律」](sop/SOP_DRAFT.md) +90 行（版本 0.20）+ 新檔 memory feedback_reverse_design.md（7 節）+ MEMORY.md +1 行
- [x] **S19 5 條教訓明文化** — (a) 逆向視角 + 正向背後觀念視角互補是雙骨架原則 / (b) Step 2 加強「不可用未推出的工具解釋自己」是普適邏輯陷阱（任何反推鏈都會踩 — 必須三處同步） / (c) 主例題切換 = 認知斷裂 / (d) PoC 模式延續 S12-S15 但迭代輪數變多（S12 Q09 1 輪 → S19 R01 3 輪 — 逆向視角比正向更易踩邏輯陷阱）/ (e) 7 鏈條樹中 R03 最關鍵（兌現 R01 暗線「①②③ 合法操作 = 左乘可逆 M」）
- [x] **S20 Mac app vs CLI 共存規則討論** — Back 問「可以一起使用嗎」；Claude 回：共用 ~/.claude/ + docs/dev/ + memory，安全用法（不同專案並行 OK / 同專案切換 OK），危險用法（同專案同時開兩邊 → CURRENT_SESSION.log race + memory 寫入打架 + auto-spawn worktree 干擾，CLAUDE.md §S113 §2.46 已警告）
- [x] **S20 Appendix E R01 + R02 領域具體化修訂 4 處** — Back 親手修 Step 1+Step 2 範例（4 處：(n)(m) 標題 / 單位 g 食材重量 / 開頭句具體化 / 從 $a_{ij}x_j$ 剝離）→ Claude 比對給「3.5/4 你贏」評 + Step 2 開頭句融合提案（修 3 處排版瑕疵：「份量$\mathbf{x}$」空格 / 「含量a」LaTeX 包 / 「加上」誤讀 + 加物件化動機鋪墊）→ Back 指示「用相同邏輯通篇修整個附錄 E」→ 4 修點：(1) R01 Step 1「結果需求」抽象化 → 領域化（食材組合份量 g + 規則固定 vs 輸入變動鋪墊）/ (2) R01 Step 2 開頭句套融合提案精修 / (3) R01 Step 4 開頭句「要還原第 i 個方程」→「第 i 個營養素的總量方程」+ 主例題前明示 b_1 / (4) R02 Step 2 開頭句加領域定位（u 收集只跟 i 有關因子 / v 收集只跟 j 有關因子 / 兩個獨立輸入維度）；R01 Step 3, 5-12 + R02 Step 0-1, 3-12 + R03-R07 預告段未動（已具體 / 預告段）
- [x] **S20 R-chain 領域具體化 4 原則明文化** — (1) 用具體領域詞非抽象 / (2) 物件化動機鋪墊「輸入 vs 規則」分工 / (3) 排版精確 / (4) 公式精準（從乘積項中剝離不是從變數中剝離）— 推 SOP §2.16 補子段「2.16 補 (S20)：R-chain 領域具體化 4 原則」+ 版本 0.21

### 進行中
- **無**（S20 已完整收工。Outstanding work 三條推 S21：**(A) Back 視覺/內文確認 R01/R02 4 處修改** → 同步 BOOK.md 重生（18 md 合一 + fence-aware awk，~10 min，必做不然 BOOK.md 與 appendix-E-reverse.md 不一致）/ **(B) 繼續寫 R03 矩陣乘法 AB 鏈**（S19 S20 兩 session 預留的核心節點，兌現 R01 暗線，預估 1h，本 session 未動因 Back 中途收工）/ **(C) S18 ch04 V-02 5 項瀏覽器驗證**（S19 至今 outstanding work，程式碼穩定可延後任何 session）。）
...
| ✅ S18 | **ch04 V-02 圖像模式 + Mona Lisa SVD 首輪交付（Back 在 S19 暫停視覺驗證 → 改寫逆向設計第二骨架，S18 視覺驗證仍 outstanding）** | viz/ch04_matrix_matrix.py 從 440 行擴至 ~570 行 / 9 cell（加 image gen + SVD_CACHE cell / mode radio 5 選項 / r_image slider 0..64 / 5 cell branch on is_small / 主舞台圖像 1×3 灰階 / strip 圖像 64 條 σ 譜柱狀圖）；WASM export 27 MB clean + http :8765；**SOP §2.15 補 S18 3 條新陷阱（合計 9 大）**：(7) 小資產 cell 內 procedural 生成 (8) [0,1] 圖像 σ_1 DC 主導 (9) random 不可 min-max |
| ✅ S19 | **逆向設計第二骨架啟動 — Appendix E R01-R02 完整鏈條落地（S18 視覺驗證暫停 → outstanding）** | 新檔 [appendix-E-reverse.md](../book/appendix-E-reverse.md) **760 行** / R01 完整 12 Step（食譜 2×3 主例題貫穿 / Step 2 加強 3 ✓ + 3 ❌ 公式可視化）+ R02 完整 12 Step（紅蘿蔔 × 盤子 → 3×2 矩陣 / 外積 uv⊤ 從「秩 1 原子」反推）+ R03-R07 預留段 + [BOOK.md](../book/BOOK.md) 12305 → **13068 行（+763 / 18 個 md 合一）** + [SCHEMA §3.7「逆向設計鏈規範」](../book/SCHEMA.md)（含 Step 2 加強鐵律 + 主例題不切換 + R01-R07 進度表，版本 0.4）+ [VIZ-CATALOG Appendix E 索引段](../book/VIZ-CATALOG.md) + 新檔 memory feedback_reverse_design.md（7 節）+ MEMORY.md +1 行 + [SOP §2.16「R-chain 寫作鐵律」](sop/SOP_DRAFT.md)（S19 痛苦教訓 ❌ ✓ 對比表 + 主例題不切換規則 + 公式可視化粒度 + R02-R07 寫作 checklist，版本 0.20）|
| ✅ S20 | **Appendix E R01+R02 領域具體化修訂 4 處 + R-chain 4 原則明文化（短收斂 session）** | Back 親手修 R01 Step 1+Step 2 範例 → Claude 比對 4 處給「3.5/4 你贏」評 + Step 2 開頭句融合提案（修 3 處排版瑕疵 + 加物件化動機鋪墊）→ Back 指示「相同邏輯通篇修」→ 4 修點：(1) R01 Step 1「結果需求」領域化 / (2) R01 Step 2 開頭句套融合提案 / (3) R01 Step 4 開頭句領域化 / (4) R02 Step 2 開頭句加領域定位 + 推 SOP §2.16 補「R-chain 領域具體化 4 原則」子段（版本 0.21）+ Mac app vs CLI 共存規則討論；BOOK.md 重生延後等 Back 確認；R03 / S18 視覺驗證均推 S21 |
| **→ S21** | **三條路任選（推薦 A 必做 + 強烈推薦 B 接著 R03）：(A) BOOK.md 同步重生（Back 視覺/內文確認 R01/R02 4 處改動後）/ (B) 繼續寫 R03 矩陣乘法 AB 鏈（兌現 R01 暗線）/ (C) S18 ch04 V-02 5 項瀏覽器驗證** | **路線 A 必做（10 min）** — Back 開 [appendix-E-reverse.md](../book/appendix-E-reverse.md) 看 4 處改動 OK 後跑 fence-aware awk 把 appendix-E-reverse.md 重新降一級 append 到 BOOK.md（13068 行 → ~13070 行 +/- few 行差）；不做的話 BOOK.md 與單檔不一致 / **路線 B 預估 1h（強烈推薦接著做）** — 從 R01 (Mv1) + 函數合成 $(AB)\mathbf{x} = A(B\mathbf{x})$ 閉合需求反推矩陣乘法 row-column 點積規則；兌現 R01 Step 2 加強的「暗線埋伏」（合法操作 ①②③ 統一為左乘可逆 M）；沿用 SOP §2.16 + SCHEMA §3.7 R02-R07 寫作 checklist + S20 新加的「R-chain 領域具體化 4 原則」；新主例題：兩個變換合成（旋轉 + 縮放 / 變數替換 + 微分 / 編碼 + 解碼之一），主例題貫穿 Step 1-9 不切換，全程用領域詞 / **路線 C 預估 0.5h + 視 console error 決定** — S19 至今 outstanding work，開 `viz/dist/ch04_matrix_matrix/index.html` 跑 5 項驗證（小矩陣模式預設 / Mona Lisa 三圖並列 / σ 譜紫灰分界線移動 / 4 圖像 σ 譜 shape 不同 / console 無 STDERR）|
| S22+ | **R04-R07 後續逆向鏈 + Python 視覺化批次旗艦實作（並行兩階段）** | 逆向設計層：R04 列空間/零空間/4 子空間（1 session）+ R05 A=CR（0.5）+ R06 偽反 A+（1）+ R07 (P4) 三明治（1）；視覺化：從 VIZ-CATALOG 次批 Tier 2（Matrix World V-01 全書教材首頁 + ch06a 五分解 dashboard 等，~28-30 session）|

### S21 起步建議（S20 已完成，Appendix E R01+R02 領域具體化修訂 + R-chain 4 原則明文化）

S20 結束時 4 處修訂落地（R01 Step 1+2+4 / R02 Step 2）+ R-chain 領域具體化 4 原則進 SOP §2.16 + Mac app vs CLI 共存規則討論。**S21 三條路（推薦 A 必做 + B 強烈推薦 + C 延後可）：**

**🅰 路線 A：BOOK.md 同步重生（必做，~10 min）**

S20 改了 [appendix-E-reverse.md](../book/appendix-E-reverse.md) 4 處（Edit diff 已 git diff 給 Back 看），但 [BOOK.md](../book/BOOK.md) 未同步重生 — 18 個 md 合一檔仍是 S19 的舊版本。**不修不一致會誤導讀者**。

S21 第一動作：
1. Back 開 [appendix-E-reverse.md](../book/appendix-E-reverse.md) 看 4 處改動（R01 Step 1 / Step 2 / Step 4 + R02 Step 2）+ 確認文字 OK
2. 若 OK，跑 fence-aware awk 把 appendix-E-reverse.md 重新降一級（# → ##）append 到 BOOK.md 末尾 — 採 S11 + S15 + S19 確立的同一流程
3. BOOK.md header「13068 行」不需改（總行數差幾行可接受），但 commit message 提一句
4. 若 Back 對某處改動不滿意，先回滾 → 再重做（保留 git history）

**🅱 路線 B：繼續寫 R03 矩陣乘法 AB 鏈（強烈推薦接著做，預估 1h）**

R03 是 7 鏈條樹中**最關鍵**的一條（S19 + S20 兩 session 都預留為下次優先）：
- **兌現 R01 暗線** — Step 2 加強埋下的「合法操作 ①②③ 後面會被統一為左乘可逆 M」承諾在 R03 兌現
- **接 R04-R07 全部** — R04 (列空間 / 零空間) 需要 R03 / R05 (A=CR) 需要 R03 / R06 (A+) + R07 (P4) 都依賴 R03

R03 寫作 checklist（沿用 SOP §2.16 R02-R07 規範 + **S20 新加的 R-chain 領域具體化 4 原則**）：

1. **Step 1 實際問題：** 兩個變換 $\mathbf{y} = B\mathbf{x}$、$\mathbf{z} = A\mathbf{y}$ 的合成（3 例子：旋轉 + 縮放 / 變數替換 + 微分運算 / 編碼 + 解碼），結果需求：找一個運算把 A, B 合成為單一矩陣 C 使 $C\mathbf{x} = A(B\mathbf{x})$
2. **新主例題（含具體數字 + 領域詞，貫穿全鏈不切換）：** 譬如 2D 旋轉 30° 後縮放 (sx=2, sy=1)，A = scale matrix, B = rotation matrix, x = (1, 0)，先算 $B\mathbf{x}$ 再 $A(B\mathbf{x})$ 取得 z；目標反推 C = AB。**用領域詞「旋轉」「縮放」貫穿，避免抽象「線性變換」**。
3. **Step 2 物件化加領域定位（S20 教訓）：** 用「輸入 vs 規則」分工論述 — 輸入 x（會變的點座標）vs 規則 A, B（固定的變換矩陣）；A 和 B 之間誰是規則誰是輸入？實際上兩者都是「變換規則」 — 鋪墊「組合兩個變換規則為新的變換規則 C」這個動機
4. **Step 2 加強對 A, B 的合法操作（嚴守 Step 2 加強鐵律 + 領域具體化）：** 只用 R01 / R02 已推出的工具（(Mv1) 點積 + 外積 uv⊤），**不可用「左乘可逆 M」這類矩陣乘法概念**（這正是我們要反推的對象）；用「合成 $(AB)\mathbf{x} = A(B\mathbf{x})$ 必須一致」當閉合需求；entry 級用旋轉/縮放具體數字驗算
5. **Step 4 反推：** $(AB)_{ij}$ 必須是 A 第 i row 與 B 第 j column 的點積（因為 $(AB)\mathbf{x} = A(B\mathbf{x})$ + R01 (Mv1) 規則）；用主例題具體數字反推
6. **Step 5-7 副產物：** AB 的 4 種讀法（行讀 / 列讀 / 行乘列 = (MM4) / 列乘行）+ 不可交換性 + 結合律 + **兌現 R01 暗線**（「對 A 做合法操作 ①②③」其實是 EA = 某種 elementary 矩陣 E 左乘 A —「左乘可逆 M」這個高層代數結構在這裡冒出，回頭看 R01 Step 2 加強的暗線埋伏段）
7. **Step 11 連結：** R04 (列空間繼承) + R07 (三明治) + R01 暗線兌現
8. **3 路 cross-link：** ch04-mat-mat.md callout 加 `🔄 逆向設計` 指 R03 + Appendix D Q09/Q10 加 R03 link + R03 Step 11 雙向

預估 ~1 小時 / 600-900 行（與 R01 同等規模）/ 寫完後三檔同步（appendix-E-reverse.md + BOOK.md 重生（路線 A）+ VIZ-CATALOG.md R03 狀態更新 + SCHEMA §3.7 R-chain 進度表）

**🅲 路線 C：S18 ch04 V-02 5 項瀏覽器驗證（延後可，S19 至今 outstanding）**

S19 至今的 outstanding work — 程式碼穩定 + WASM export clean。S21 可選擇先把這個驗證跑完：

0. **跑 5 項瀏覽器驗證清單** — 開 `viz/dist/ch04_matrix_matrix/index.html`（或起 http.server）測：(1) 小矩陣模式預設正常顯示 / (2) 切 Mona Lisa：三圖並列「原圖看得到臉？/ r 小看不到 / r 大清楚」 / (3) 拉 r 0..64：重建精緻化 + σ 譜柱狀圖紫/灰分界線移動 / (4) 依序切條紋/漸層/隨機：σ 譜 shape 明顯不同 / (5) Console DevTools 無 STDERR / Traceback
1. **OK 才進入 Tier 3 補完**（飛入動畫 800ms / 4 重排序 / 誤差曲線 / Walkthrough 6 步 / 快捷鍵），詳見下方 S19 歷史保留段
2. **有 console error 先修**

預估 ~0.5h 驗證 + Tier 3 補完 1-1.5 session

**S21 強烈建議走 A + B 連動**：A 必做（10 min 解一致性），B 是核心節點（寫完 R04-R07 全部解鎖）。**S21 預估總 1.5-2h**（A 10min + B 1h + 收工 0.5h）。

### S20 起步建議（S19 已完成，逆向設計第二骨架啟動 — R01-R02 完成 / R03-R07 預留）— 歷史保留

S19 結束時 R01 PoC 通過 + R02 完成 + 三處規範同步（SOP §2.16 + SCHEMA §3.7 + memory feedback_reverse_design.md）。**S20 兩條路 Back 任選：**

**🅰 路線 A：繼續寫 R03 矩陣乘法 AB 鏈（推薦 — 兌現 R01 暗線）**

R03 是 7 鏈條樹中**最關鍵**的一條：
- **兌現 R01 暗線** — Step 2 加強埋下的「合法操作 ①②③ 後面會被統一為左乘可逆 M」承諾在 R03 兌現
- **接 R04-R07 全部** — R04 (列空間 / 零空間) 需要 R03 矩陣乘法 / R05 (A=CR) 需要 R03 / R06 (A+) + R07 (P4) 都依賴 R03

R03 寫作 checklist（沿用 SOP §2.16 R02-R07 規範）：

1. **Step 1 實際問題：** 兩個變換 $\mathbf{y} = B\mathbf{x}$、$\mathbf{z} = A\mathbf{y}$ 的合成（3 例子：旋轉 + 縮放 / 變數替換 + 微分運算 / 編碼 + 解碼），結果需求：找一個運算把 A, B 合成為單一矩陣 C 使 $C\mathbf{x} = A(B\mathbf{x})$
2. **新主例題（含具體數字，貫穿全鏈）：** 譬如 2D 旋轉 30° 後縮放 (sx=2, sy=1)，A = scale matrix, B = rotation matrix, x = (1, 0)，先算 $B\mathbf{x}$ 再 $A(B\mathbf{x})$ 取得 z；目標反推 C = AB
3. **Step 2 加強對 A, B 的合法操作（嚴守鐵律）：** 只用 R01 / R02 已推出的工具（(Mv1) 點積 + 外積 uv⊤），**不可用「左乘可逆 M」這類矩陣乘法概念**（這正是我們要反推的對象）；用「合成 $(AB)\mathbf{x} = A(B\mathbf{x})$ 必須一致」當閉合需求
4. **Step 4 反推：** $(AB)_{ij}$ 必須是 A 第 i row 與 B 第 j column 的點積（因為 $(AB)\mathbf{x} = A(B\mathbf{x})$ + R01 (Mv1) 規則）
5. **Step 5-7 副產物：** AB 的 4 種讀法（行讀 / 列讀 / 行乘列 = (MM4) / 列乘行）+ 不可交換性 + 結合律 + **兌現 R01 暗線**（「對 A 做合法操作 ①②③」其實是 EA = 某種 elementary 矩陣 E 左乘 A —「左乘可逆 M」這個高層代數結構在這裡冒出）
6. **Step 11 連結：** R04 (列空間繼承) + R07 (三明治) + R01 暗線兌現
7. **3 路 cross-link：** ch04-mat-mat.md callout 加 `🔄 逆向設計` 指 R03 + Appendix D Q09/Q10 加 R03 link + R03 Step 11 雙向

預估 ~1 小時 / 600-900 行（與 R01 同等規模）/ 寫完後三檔同步（appendix-E-reverse.md / BOOK.md / VIZ-CATALOG.md / SCHEMA.md R-chain 進度表）

**🅱 路線 B：回 S18 ch04 V-02 視覺驗證原路線**

S18 ch04 V-02 視覺驗證仍是 outstanding work — 程式碼 + WASM export clean，但 Back-side 5 項瀏覽器驗證未完成（在 S19 開頭 Back 改方向到逆向設計）。S20 可選擇先把這個驗證跑完：
...
**S20 強烈建議走 A**：R03 是逆向設計鏈的「核心節點」，寫完後 R04-R07 全部解鎖；S18 視覺驗證 outstanding work 可以延後到任何 session 處理（程式碼穩定不會壞）。但決定權在 Back。

### S19 起步建議（S18 等視覺確認後再進入 Tier 3 補完）— 歷史保留

0. **S19 第一動作：請 Back 跑 S18 5 項瀏覽器驗證清單** — 開 `viz/dist/ch04_matrix_matrix/index.html`（或起 http.server）測：(1) 小矩陣模式預設正常顯示 / (2) 切 Mona Lisa：三圖並列「原圖看得到臉？/ r 小看不到 / r 大清楚」 / (3) 拉 r 0..64：重建精緻化 + σ 譜柱狀圖紫/灰分界線移動 / (4) 依序切條紋/漸層/隨機：σ 譜 shape 明顯不同（條紋只 1 根 / 漸層 2 根 / 隨機緩降） / (5) Console DevTools 無 STDERR / Traceback。**OK 才進入 S19；有 console error 先修。**

1. **S19 任務：ch04 V-02 Tier 3 補完** — 接 S18 完成的雙模式（小矩陣 + 4 圖像）骨架，加飛入動畫 / 重排序 / 誤差曲線 / Walkthrough / 快捷鍵；預估 1-1.5 session

2. **S19 拆 5 步（在 Back 驗證 OK 後執行）：**
   - **Step 1（~25 min）：** 飛入動畫 — r slider 上移時新增的秩 1 圖層從 strip 位置「飛到」主舞台中央 + 與 Cr 合併
     - 800 ms / 項 + ease-in-out cubic-bezier
     - 圖像模式（k=64）動畫量太大不適合每項飛，改為「最近一次增加的圖層」高亮 + 透明度脈動 800 ms
     - 技術選項 1：plotly frames + animation_frame（complex，與 reactive cell 整合難）
     - 技術選項 2（推薦）：marimo `mo.ui.run_button` + CSS transition 動畫，每次按下播放 0..r 累加動畫
   - **Step 2（~25 min）：** 4 重排序 radio — 鋪陳 Eckart-Young
     - radio options: 「按 σ_p 排序」/「按 ||a_p|| 排序」/「隨機順序」/「自訂順序」
     - 按 σ_p 排序時誤差曲線下降最快，這是 SVD 最佳低秩近似 Eckart-Young 定理的視覺核心
     - 小矩陣模式 k=2 重排序意義有限（只有 1 種 vs 反序），主要在圖像模式 k=64 才有戲
   - **Step 3（~20 min）：** 誤差曲線圖 r vs ||M - Mr||_F / ||M||_F
     - plotly Scatter + lines + markers，所有 0..k 點計算一次
     - 當前 r 紅點 + 垂直虛線標記
     - 圖像模式對比 4 重排序時，4 條曲線同圖上覆蓋，「按 σ_p 排序」是下降最快的那條（漂亮的 Eckart-Young 視覺）
   - **Step 4（~15 min）：** Walkthrough 6 步首次開啟自動觸發
     - 用 marimo `mo.ui.text_area` 或 markdown overlay 顯示步驟
     - 6 步：(1) 看 A B 兩矩陣 / (2) 拉 r=0 全 0 / (3) r=1 第 1 層 / (4) r=2 完整 / (5) 切 Mona Lisa / (6) r=5 已辨識
     - 用 localStorage 或 marimo state 記首次開啟標誌
   - **Step 5（~10 min）：** 快捷鍵 Space / ←/→ / R / 0 / Shift+End / M
     - Space 播放動畫 / ←→ r±1 / R 重設 / 0 r=0 / Shift+End r=k / M 切換 mode
     - marimo 支援 keyboard shortcut 但綁定方式需查文件（可能要寫 JS 注入）

3. **S19 開工 checklist（SOP §2.15 9 大陷阱沿用）：**
   - [ ] 寫程式前先列「pedagogical 預期表」對拍每張圖 σ 譜形狀 + 關鍵閾值（S18 教訓）
   - [ ] 寫教學表前先實際算數字不憑估計（S18 教訓）
   - [ ] 跨 cell 共用的函數 / 變數命名不可 `_` 開頭（§2.15 第 4 條）
   - [ ] plotly `subplot_titles` 不可傳空字串（§2.15 第 5 條）
   - [ ] heatmap helper 內 `M = np.asarray(M, dtype=float)` 保護（§2.15 第 6 條）
   - [ ] 小資產 ≤ 100 KB 一律 in-notebook procedural 生成（§2.15 第 7 條）
   - [ ] WASM export 後 console 必看 STDERR + Loading 順序

4. **參考工件：**
   - [viz/ch04_matrix_matrix.py](../../viz/ch04_matrix_matrix.py) — S18 雙模式骨架（已 work，S19 直接擴）
   - [viz/_common/rank1_layer.py](../../viz/_common/rank1_layer.py) — 工具函數本機 reference（注意 WASM 仍要 inline）
   - [VizScript-02 in ch04-mat-mat.md](../book/ch04-mat-mat.md#vizscript-02) §E 動畫 + §F 快捷鍵 + §G Walkthrough 規格 — 直接參照
   - [SOP_DRAFT.md §2.15](sop/SOP_DRAFT.md) — 9 大 WASM 陷阱

5. **S19 後續：** Tier 3 完成後 S20+ 進入次批 Tier 2 旗艦（Matrix World V-01 全書互動式教材首頁 + ch06a V-01 五分解 dashboard + ch06b/c/d/e V-01 五分解 pointer，依 S17-S19 母模板複製樣板，預估 ~28-30 session）
...
| 2026-05-15 | **S19 確立「逆向設計視角為全書第二骨架」** | Back 提出全書除「正向背後觀念」（Appendix D 3-layer）外，須建立「逆向設計鏈」（Appendix E R01-R07）作第二骨架。**Why（為什麼可行）：** 矩陣運算（點積 / 外積 / 矩陣乘法 / 列空間 / 4 子空間 / 五大分解 / 偽反）**沒有一個是憑空發明的**，全部是某個「實際問題 + 還原需求」反推出來的**唯一可能規則** — 所以「反推鏈」不是虛構故事，而是合法的數學推導路徑。**Why（為什麼需要）：** 正向視角（給規則 → 解釋意義）即使配上 Appendix D 的「為什麼」歷史脈絡與設計過程還原，仍然是「規則先存在，再回頭找動機」的後驗解釋。逆向視角是「規則尚未存在，我們從零反推它必須長什麼樣」 — 這是真正的「設計思維 / 工程師思維」。兩個視角互補：D 從橫切面剖析（規則確立後的多層意義），E 從縱向走一遍（規則從零產生的設計過程）。三處同步明文化（SOP §2.16 + SCHEMA §3.7 + memory feedback_reverse_design.md）|
| 2026-05-15 | **S19 確立「R-chain 5 步反推骨架 + 12 Step 標準段落」** | 5 步骨架：(1) 實際問題（結果需求）/ (2) 物件化（抽離係數與變數，含可逆閉合條件）/ (3) 尋找未知運算 ◯ / (4) 閉合需求反推 ◯ 的規則 / (5) 副產物自動冒出。展開為 12 Step：Step 0 我們在做什麼 / 1 實際問題 / 2 物件化 / 2 加強對物件合法操作 / 3 尋找 ◯ / 4 反推 ◯ 規則 / 5-7 副產物 1-3 / 8 雙路閉合驗證 / 9 邊界條件 / 10 昇華 / 11 連結其他 R-chain / 12 Strang 鎖核。每條 R-chain 預估 600-900 行 / ~35 個 boxed 公式 / ~20 個具體驗算 / 主例題貫穿 Step 1-9 不切換 |
| 2026-05-15 | **S19 確立「R-chain Step 2 加強鐵律：不可用未推出的工具解釋自己」** | S19 R01 PoC 第一稿犯邏輯循環錯誤：用「左乘可逆 M / elementary 矩陣 / LU 分解」描述合法操作 → Back 立即指出「跳太遠 邏輯循環」（要先懂矩陣乘法 = 整條鏈反推目標）→ 修訂改用「**方程組層代數語言**」（中學代數變形：方程乘 c / 互換 / 加倍）+ entry 級同步動作 + 暗線埋伏指 R03 兌現 → Back 確認「符合精神」。**規範：寫 R-chain Step 2 加強描述「對物件的合法操作」絕對不可使用尚未在 Step 4 反推出來的運算工具來解釋**；若需承諾後續鏈兌現，用「暗線埋伏」段（quote block）指向**後續**鏈條而**不**在當前 Step 展開；R-chain 寫完倒過來檢查 Step 2 加強用到的每個術語是否在 Step 4 之前就成立 |
| 2026-05-15 | **S19 確立「R-chain 主例題不切換規則」** | S19 R01 第二稿時為簡化把主例題 2×3 切換到 2×2 副例題寫 Step 2a/2b → Back 立即制止「請想辦法延續用 Step 1 的例子不要換到 2×2」（**例題切換 = 認知斷裂**：讀者好不容易記住食譜 2×3 系統，突然要追蹤新 2×2 系統 → 心智 context 重置成本高，破壞鏈條閱讀體驗）。**規範：R-chain 必須有一個主例題（含具體數字）貫穿 Step 1-9，不可中途切換到新例題**（除非極有理由，且要明確標示）；S19 R01 修訂後全程用食譜 2×3 主例題 |
| 2026-05-15 | **S19 確立「R-chain 公式可視化粒度」** | Back 原話：「R01 鏈需要把每一步你提到的操作都用公式推導可視化」。**規範：** 每個操作步驟都要 boxed 公式 + 具體 entry 級數字 + 驗算對拍（不能只是文字描述）。R01 完成版每個操作都有：(1) 方程組層代數變形（具體展開含 boxed 公式）/ (2) A 與 b 的 entry 級同步動作（具體新矩陣 / 向量）/ (3) 驗證原解 $\mathbf{x}^*$ 是否仍是新系統的解（代入具體驗算）/ (4) 說明為何合法 / 為何破壞（與閉合需求對照）。R01 整體 ~35 個 `$$\boxed{...}$$` 公式塊 / ~20 個具體驗算 — 這是逆向鏈的標準粒度 |
| 2026-05-15 | **S19 暫停 S18 視覺驗證路線，改方向到逆向設計第二骨架** | Back 在 S19 開頭把原 S19 路線（先跑 5 項瀏覽器驗證再進入 ch04 V-02 Tier 3 補完）**暫停**，提出逆向設計第二骨架；S18 視覺驗證仍是 outstanding work（程式碼穩定 + WASM export clean），未來 S20+ 任何 session 都可回頭跑驗證 + Tier 3 補完。**規則：** 大方向轉折時，原路線「暫停」而非「取消」，明確標記 outstanding work 待後續 session 處理；HANDOFF 結構容納兩條並行路線（Appendix E 逆向設計層 + ch04 V-02 視覺驗證 + Tier 3）|
| 2026-05-15 | **S19 確立「PoC 模式延續但迭代輪數變多」** | S12 Q09 PoC 1 輪通過（正向背後觀念寫作）→ S19 R01 PoC 3 輪才通過（逆向設計寫作）。**原因：逆向視角比正向更易踩邏輯陷阱**（譬如「不可用未推出的工具解釋自己」這個陷阱在正向視角不存在 — 正向先有規則再解釋，不會循環；逆向是反推規則，極容易誤用「規則本身」當解釋工具）。**S20+ R02-R07 寫作要預期 2-3 輪迭代而非 1 輪通過**；遇到 Back 制止「跳太遠 / 切換例題 / 邏輯循環」要立即接受並修訂，不要辯護 |
| 2026-05-15 | **S19 確立「7 鏈條樹中 R03 最關鍵，建議 S20 開頭優先寫」** | R03 = 矩陣乘法 AB 從「函數合成」需求反推，是 7 鏈條樹的**核心節點**：(a) 兌現 R01 Step 2 加強埋下的「合法操作 ①②③ 後面會被統一為左乘可逆 M」暗線 (b) R04-R07 全部依賴 R03（R04 列空間 / R05 A=CR / R06 偽反 / R07 (P4) 三明治）。S20 強烈建議走 R03 路線；S18 視覺驗證 outstanding work 可以延後到任何 session 處理 — 程式碼穩定不會壞 |
| 2026-05-16 | **S20 確立「R-chain 領域具體化 4 原則」（S19 R-chain 寫作鐵律延伸）** | S20 Back 親手修 R01 Step 1+Step 2 範例展示「具體領域詞 + 輸入 vs 規則分工 + 排版精確 + 公式精準」風格 → Claude 推論出 4 條通用原則套全文。**4 原則：(1) 領域具體詞** 用「食材/營養素/份量/含量」具體詞而非「結果分量/係數/未知量」抽象詞（讀者腦補成本低，配主例題情境）/ **(2) 物件化動機鋪墊「輸入 vs 規則」分工** 寫物件化前先說「哪部分會變（輸入）哪部分固定（規則）」— 為「為什麼某物該被剝離成獨立物件」鋪墊邏輯（R01 主例題：x 是會變的份量、a 是固定的食材成分表規則 → 抽離 a 成 A 才有邏輯動機）/ **(3) 排版精確** LaTeX 變數正確包裹、變數與單位空格、不重複定義（如「份量$\mathbf{x}$ $(x_1, \ldots, x_n)$」是重複，$\mathbf{x}$ 已含分量定義）/ **(4) 公式精準** 「從 $a_{ij}x_j$ 中剝離」（從乘積項中剝離 a）正確；「從 $x_j$ 中剝離」錯（係數不在 $x_j$ 裡）。**為什麼分 4 條而非 1 條：** 4 條是獨立可違反的維度（領域具體可、但動機沒鋪墊；動機鋪墊可、但排版瑕疵），4 條都要過才算高品質 R-chain 段落。**補進 SOP §2.16「2.16 補 (S20)：R-chain 領域具體化 4 原則」子段（版本 0.21）**；R03-R07 寫作 checklist 升級為 8 條（原 7 條 + 第 8 條「全段套 R-chain 領域具體化 4 原則」）|
| 2026-05-16 | **S20 確立「Mac app vs CLI 共存使用規則」** | Back 問「Claude Mac app vs CLI 可以一起使用嗎」。**共用部分（兩邊看同一份）：** ~/.claude/settings.json + ~/.claude/CLAUDE.md + ~/.claude/projects/<project>/memory/ + 專案內 docs/dev/HANDOFF.md / SESSION_INDEX.md / CURRENT_SESSION.log。**安全用法：** (a) Mac app 開 A 專案、CLI 開 B 專案 — 完全並行 OK / (b) 切換使用（Mac app 收工 → 換 CLI 開同一專案）— HANDOFF.md 是純文字交接兩邊都讀同一份。**危險用法：** 同一個專案、同一個時間、兩邊都開 — 會撞 race：CURRENT_SESSION.log 互相覆寫 / memory 寫入打架 / auto-spawn worktree 干擾（CLAUDE.md S113 §2.46 已警告「Mac app→CLI 切換後仍保留 worktree 檢查」是因 Mac app 改版會 auto-spawn）。**規則：跨工具切換用 HANDOFF.md，並行用不同專案；同專案禁同時開兩邊**。本條僅是討論結論，不需專門記錄至 SOP（屬使用環境層而非開發流程層）|
| 2026-05-16 | **S20 確立「短收斂 session 也算完整 session」** | S20 全 session 沒有大量產出（vs S15 3.5h / S19 4h），只 ~30 min（4 處 Edit + 4 原則明文化 + Mac app 討論 + 收工），但完成「相同邏輯通篇套」高密度精修 — 效率高且任務完整。**規則：短 session 收工流程不縮水**（HANDOFF / SESSION_INDEX / CURRENT_SESSION / RETROSPECTIVE / SOP / git commit + push 全做），保持跨 session 交接無縫銜接 — S20 出現「Back 中途主動收工要求詳細打包」這種觸發詞時更該完整跑收工 |
...
| 檔案 | 動作 | 說明 |
|------|------|------|
| docs/book/appendix-E-reverse.md | **新增** | **760 行 / 38 KB** — Appendix E 逆向設計層；R01 完整 12 Step（食譜 2×3 主例題貫穿 / Step 0-12 / Step 2 加強 3 ✓ + 3 ❌ 公式可視化 / Step 4 反推點積規則 boxed / 副產物 (Mv2) + 列空間 + rank + Rank-Nullity 自動冒出 / Step 9 線性 vs 非線性邊界 / Step 11 R02-R07 鏈條樹 / Step 12 Strang 鎖核）+ R02 完整 12 Step（紅蘿蔔 × 盤子 → 3×2 矩陣 / 外積 uv⊤ 從「秩 1 原子」反推 / 副產物形狀規則 + rank 1 + 矩陣秩 1 拆解 MM4 / SVD 之源）+ R03-R07 預留段（每段含預定鏈條 5 步骨架預告）|
| docs/book/BOOK.md | 修改 | 12305 → **13068 行（+763）** — fence-code-aware awk 把 appendix-E-reverse.md 降一級（# → ##）append 到 BOOK.md 末尾；同時更新 header「17 個原 md 檔」→「18 個原 md 檔」+ 目錄表加 Appendix E 一行 |
| docs/book/SCHEMA.md | 修改 | **新增 §3.7「逆向設計鏈（R-chain）規範」段（S19 確立，版本 0.4）** — 觸發時機 + 5 步反推骨架 + 12 Step 段落表 + **「Step 2 加強鐵律」**（禁用未推出的工具解釋自己 + S19 ❌ 反例 ✓ 正例對比）+ 主例題不切換規則 + 公式可視化粒度 + 篇幅規範 + 3 路 cross-link 規範（主章 ↔ Appendix D Q&A ↔ Appendix E R-chain）+ R01-R07 全書進度表 |
| docs/book/VIZ-CATALOG.md | 修改 | **新增「Appendix E 逆向設計 R01-R07 鏈索引」獨立段** — 7 條鏈完整列表（R01-R07）+ 對應主章 + 對應 Q&A + 連結 + 狀態 + 5 步反推骨架說明 + 核心信念 + 3 路 cross-link + 三條閱讀路線（正向接逆向 / 直接逆向 / 主題追蹤） + 變更歷史補 S19 行 |
| .claude/projects/.../memory/feedback_reverse_design.md | **新增** | **memory** — 「逆向設計視角為全書第二骨架」明文化（Why + How to apply A-G 7 節含 5 步骨架 / 12 Step / Step 2 加強鐵律 / 公式可視化 / 主例題貫穿 / 3 路 cross-link / R01-R07 路線）|
| .claude/projects/.../memory/MEMORY.md | 修改 | +1 行 index 指向 feedback_reverse_design.md |
| docs/dev/sop/SOP_DRAFT.md | 修改 | **新增 §2.16「逆向設計鏈（R-chain）寫作鐵律」段（S19 確立，版本 0.20）** — S19 痛苦教訓明文化（第一稿邏輯循環錯誤的 ❌ 反例 + 修訂後 ✓ 正例對比表）+ 主例題不切換規則（Back 制止 2×2 副例題切換 原話保留）+ 公式可視化粒度（每步操作都 boxed 公式 + entry 級數字 + 驗算）+ 暗線埋伏規範 + R02-R07 寫作 checklist |
| docs/dev/CURRENT_SESSION.log | 修改 | S19 啟動 + 即時記錄（Back 改方向 / R01 PoC 3 輪迭代 / R02 完成 / 7 檔同步收尾） + 結束記錄 |
| docs/dev/SESSION_INDEX.md | 修改 | 追加 S19 一行（含 R01-R02 完整內容 + 痛點記錄 + 5 條觀察 + S20 兩條路）|
| docs/dev/HANDOFF.md | 修改 | 整檔覆寫（最後更新 → S19 / 已完成追加 S19 10 條 / 進行中改「無 / S18 outstanding」 / 待辦表 S19 行改「✅ 完成」+ 加 S20 行 / S20 起步建議完整新寫 + S19 起步歷史保留 / 關鍵決策追加 7 條 / 檔案變更追蹤追加 S19 區）|

### S20 新增/修改的檔案

| 檔案 | 動作 | 說明 |
|------|------|------|
| docs/book/appendix-E-reverse.md | 修改 | **R01 + R02 領域具體化修訂 4 處** — (1) R01 Step 1 line 91-93「結果需求」抽象化 → 領域化（食材組合份量 g + 規則固定 vs 輸入變動鋪墊為下一步物件化埋伏）/ (2) R01 Step 2 line 97 開頭句套融合提案精修（3 處排版瑕疵：「份量$\mathbf{x}$」空格 / 「含量a」LaTeX 包 / 「加上」誤讀 + 明確「兩部分決定」分工：輸入會變 vs 規則不變 + 串「→ 物件化動機是抽離不變的規則」）/ (3) R01 Step 4 line 302-308 開頭句「要還原第 i 個方程」→「第 i 個營養素的總量方程」+ 主例題前明示「用熱量 b_1（A 第 1 row × 份量 x）」/ (4) R02 Step 2 line 512-518 開頭句加領域定位（u 收集只跟 i 有關因子（單顆熱量隨 size 變）/ v 收集只跟 j 有關因子（顆數隨 j 變）+ 兩個向量是互相獨立的兩個輸入維度 + 加「i, j 兩個獨立索引各自從 u, v 取一個分量配對相乘」括註）；另 Back 預先修 3 處（標題加 (n)(m) / 單位 g, 食材重量 / 從 $a_{ij}x_j$ 中剝離）合計 7 處改動；**BOOK.md 未同步重生**（推 S21 由 Back 視覺確認 4 處後執行）|
| docs/book/BOOK.md | （pending S21）| **未同步更新** — 仍是 S19 13068 行版本，與 appendix-E-reverse.md 不一致；S21 路線 A 由 Back 視覺確認 4 處修改 OK 後跑 fence-aware awk 重生 |
| docs/dev/sop/SOP_DRAFT.md | 修改 | **§2.16 補「2.16 補 (S20)：R-chain 領域具體化 4 原則」子段** — (1) 領域具體詞 / (2) 物件化動機「輸入 vs 規則」分工 / (3) 排版精確 / (4) 公式精準；R03-R07 寫作 checklist 升級為 8 條（原 7 條 + 第 8 條「全段套 R-chain 領域具體化 4 原則」）+ 版本記錄追加 0.21 |
| docs/dev/CURRENT_SESSION.log | 修改 | S20 啟動（已寫於 session 開頭）+ 即時記錄（Mac app vs CLI 討論 / Back 修訂範例比對 / Back 指示通篇修 / 4 修點逐個落地 / git diff 呈 Back / Back 中途收工要求詳細打包）+ 結束記錄 |
| docs/dev/SESSION_INDEX.md | 修改 | 追加 S20 一行（短收斂 session：4 處修訂 + 4 原則明文化 + Mac app vs CLI 規則）|
| docs/dev/HANDOFF.md | 修改 | 整檔覆寫（最後更新 → S20 / 已完成追加 S20 4 條 / 進行中改「無 / 3 outstanding work 推 S21」 / 待辦表 S20 行改「✅ 完成」+ 加 S21 行 + S22+ 行 / S21 起步建議完整新寫含 A/B/C 三路 + S20 起步歷史保留 / 關鍵決策追加 4 條 / 檔案變更追蹤追加 S20 區）|
| docs/dev/RETROSPECTIVE.md | 修改 | S20 對話反思追加（短 session / Back 親手範本反推「相同邏輯」協作模式 / 4 修點 TodoWrite 規劃 / 收工要求觸發完整打包流程）|
...
```
請先讀取以下檔案恢復 context：
1. docs/dev/HANDOFF.md — 上次 session 狀態（本檔，S19 收工版）
2. docs/dev/SESSION_INDEX.md — 歷史 session 列表（含 S19）
3. docs/dev/CURRENT_SESSION.log — 上一次 session 即時記錄
4. **docs/book/appendix-E-reverse.md — S20 開工首要檔（逆向設計 R01-R07 鏈，R01-R02 完成 / R03-R07 預留；S20 兩條路任選：A 寫 R03 矩陣乘法 / B 跑 S18 ch04 V-02 5 項瀏覽器驗證）**
5. docs/book/appendix-D-why.md — 全書 22 Q&A（S15 完成 100%）— 對偶閱讀 R-chain 時對照
6. docs/book/VIZ-CATALOG.md — 全書 36 個 VizScript metadata 索引 + S19 新增 Appendix E R01-R07 鏈索引
7. docs/book/BOOK.md — 全書合併單檔（**13068 行 / 18 個 md 合一**，整本書離線閱讀版；含 Appendix E）
8. docs/book/SCHEMA.md（§3.5 全書視覺錨點 / §3.6 背後觀念 callout 規範 / §3.7 逆向設計鏈 R-chain 規範 — S19 新增）+ docs/book/VIZ_SCHEMA.md
9. docs/dev/sop/SOP_DRAFT.md（含 S00-S19 全部章節寫作策略 + 背後觀念層 3-layer 框架 §2.13 + **逆向鏈寫作鐵律 §2.16 — S19 新增**）
10. docs/dev/RETROSPECTIVE.md — Session 對話反思（含 S18 條目）
然後從「S20 起步建議」決定本次任務：

**🅰 推薦路線 A：寫 R03 矩陣乘法 AB 鏈**（預估 ~1h / 600-900 行）
- **動機：** 兌現 R01 Step 2 加強「①②③ 合法操作 = 左乘可逆 M」暗線；R04-R07 全部依賴 R03（核心節點）
- **5 步骨架：**
  - Step 1 實際問題：兩個變換 y=Bx, z=Ay 的合成（3 例子：旋轉+縮放 / 變數替換+微分 / 編碼+解碼）
  - Step 2 物件化 + 加強：閉合需求 (AB)x = A(Bx) 必須一致；**嚴守 SOP §2.16 鐵律**只用 R01/R02 已推出工具（(Mv1) 點積 + 外積 uv⊤）描述合法操作，**不可用尚未推出的「左乘可逆 M」**
  - Step 4 反推：(AB)_ij 必須是 A 第 i row 與 B 第 j column 的點積（從 (AB)x = A(Bx) + R01 (Mv1) 強制反推）
  - Step 5-7 副產物：AB 4 種讀法（行讀 / 列讀 / 行乘列=MM4 / 列乘行）+ 不可交換性 + 結合律 + **兌現 R01 暗線**（左乘可逆 M 在此冒出）
  - Step 11 連結：R04 列空間繼承 / R07 三明治 / R01 暗線兌現
- **主例題建議：** 2D 旋轉 30° + 縮放 (sx=2, sy=1) 兩變換合成，x = (1, 0)，先算 Bx 再 A(Bx)，反推 C = AB
- **3 路 cross-link：** ch04-mat-mat.md callout 加 🔄 link / Appendix D Q09/Q10 加 R03 link / R03 Step 11 雙向
- **S19 觀察：** R-chain 寫作預期 2-3 輪迭代而非 1 輪通過；遇到 Back 制止「跳太遠 / 切換例題 / 邏輯循環」要立即接受並修訂
- **沿用 SOP §2.16 R02-R07 寫作 checklist：** 7 條規範完整列在 SOP §2.16 末尾

**🅱 路線 B：回 S18 ch04 V-02 視覺驗證原路線**（預估 0.5h 驗證 + 1-1.5 session Tier 3 補完）
- 開 `viz/dist/ch04_matrix_matrix/index.html` 跑 5 項驗證清單（小矩陣模式 / Mona Lisa 三圖並列 / σ 譜紫灰分界線移動 / 4 圖像 σ 譜 shape 不同 / console 無 STDERR）
- OK 才進入 Tier 3 補完（飛入動畫 800ms / 4 重排序 / 誤差曲線 / Walkthrough 6 步 / 快捷鍵）
- 詳見 HANDOFF S19 起步建議（歷史保留段）

**S20 強烈建議走 A**：R03 是逆向設計鏈核心節點，寫完 R04-R07 全解鎖；S18 視覺驗證 outstanding work 可延後（程式碼穩定不會壞）。但決定權在 Back。
```
