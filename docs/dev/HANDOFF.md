...
### 進行中
- **S18 等 Back 視覺確認**：S18 = 首輪交付（程式碼 + 本機 SVD 對拍通過 + WASM export clean + http.server :8765 已啟動），**Back-side 5 項瀏覽器驗證未完成**（Back 在我發 5 項清單後立即收工）。下次 S19 開頭該請 Back 開瀏覽器跑 5 項驗證清單（在 [HANDOFF S19 起步建議](#s19-起步建議-s18-等視覺確認後再進入-tier-3-補完) 段），OK 才進入 S19 補完 Tier 3（飛入動畫 800ms / 4 重排序對比 / 誤差曲線 / Walkthrough 6 步 / 快捷鍵）；有 console error 先修
...
| ✅ S18 | **ch04 V-02 圖像模式 + Mona Lisa SVD 首輪交付（等 Back 視覺確認）** | viz/ch04_matrix_matrix.py 從 440 行擴至 ~570 行 / 9 cell（加 image gen + SVD_CACHE cell / mode radio 5 選項 / r_image slider 0..64 / 5 cell branch on is_small / 主舞台圖像 1×3 灰階 / strip 圖像 64 條 σ 譜柱狀圖）；**設計決策跳過 HANDOFF S17 預估「WASM 讀 npy 不確定性」** 改程式內 procedural 圖像生成（portrait / stripes / gradient / random raw standard_normal） + 即時 SVD（~5 ms × 4 = ~20 ms）；本機對拍 4 圖預期性質符合（portrait σ_1=42.28 eff_rank 5 / stripes rank 1 精確 / gradient rank 2 精確 / random Marchenko-Pastur r=28 才達 85% 能量）；設計失誤自我修正（random min-max 注入 DC → raw centered）+ footer 用實測數字校正 + DC 注意段；WASM export 27 MB clean + http :8765；**SOP §2.15 補 S18 3 條新陷阱（合計 9 大）**：(7) 小資產 ≤ 100 KB cell 內 procedural 生成 (8) [0,1] 圖像 σ_1 DC 主導累積能量與視覺品質不對齊 (9) random 不可 min-max（保留 raw centered 才有 Marchenko-Pastur σ 譜）|
| **→ S19** | **ch04 V-02 Tier 3 補完（飛入動畫 + 重排序 + 誤差曲線 + Walkthrough）** | **S19 第一動作該請 Back 5 項瀏覽器驗證清單跑完 S18**（小矩陣模式預設正常 / 切 Mona Lisa 三圖並列 / σ 譜紫灰分界線移動 / 切 4 圖像 σ 譜 shape 不同 / console 無 STDERR），OK 才進入 Tier 3 補完：(1) r slider 移動時秩 1 圖層從 strip「飛到」主舞台中央 + 與 Cr 合併動畫 800ms / 項 + ease-in-out cubic-bezier (2) 4 重排序 radio（按 σ_p / ||a_p|| / 隨機 / 自訂）對比，按 σ_p 排序時誤差曲線下降最快 — 鋪陳 Eckart-Young §6.5 (3) 誤差曲線圖 r vs ||C-Cr||_F / ||C||_F 即時更新，當前 r 紅點 + 垂直虛線標記 (4) Walkthrough 6 步首次開啟自動觸發 (5) 快捷鍵 Space / ←/→ / R / 0 / Shift+End / M；預估 1-1.5 session |
| S20+ | Python 視覺化批次旗艦實作 | 從 VIZ-CATALOG 次批 Tier 2（含 Matrix World V-01 全書互動式教材首頁 + ch06a V-01 五分解 dashboard 等），預估 ~28-30 session |

### S19 起步建議（S18 等視覺確認後再進入 Tier 3 補完）
